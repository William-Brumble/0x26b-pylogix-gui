...
@dataclass(kw_only=True)
class ConnectionSize(Request):
    connection_size: int

@dataclass(kw_only=True)
class ConnectionSizeResponse:
    connection_size: int
...
@dataclass(kw_only=True)
class PlcTime(Request):
    raw: bool = False


@dataclass(kw_only=True)
class TagList(Request):
    all_tags: bool = True


@dataclass(kw_only=True)
class ProgramTagList(Request):
    program_name: str


@dataclass(kw_only=True)
class ModuleProperties(Request):
    slot: int
...
class ResponseAsDataclass:
    TagName: str | None
    Value: bool | int | str | list[str] | float | datetime | list[
        Tag
    ] | LGXDevice | list[LGXDevice] | None
    Status: str | None
