...
from models.models import StatusDTO
...
def common_exception_handler(f):
    ...
    def modified_f(self, *args, **kwargs):
        try:
            ...
            self.logger.error(f"exception in method {f.__name__}: {e}")
            payload = asdict(
                StatusDTO(
...
def common_connection_protection(f):
    ...
    def modified_f(self, *args, **kwargs):
        self.logger.debug("Checking to see if there is a connection to a PLC")
        if self._plc.IPAddress != "":
            self.logger.debug("There is a connection to a PLC all is well!")
            return f(self, *args, **kwargs)
        else:
            self.logger.warning(
    ...
    def wrap(f):
        def modified_f(self, *args, **kwargs):
            ...
            try:
                ...
                self.logger.debug(f"Needed payload fields: {payload_class_field_names}")
                ...
                self.logger.debug("Payload matches all is well")
...
def common_token_protection(f):
    ...
    def wrap(self, *args, **kwargs):
        ...
        self.logger.debug(f"Request token: {token}")
        self.logger.debug(f"Server token: {webview.token}")

        if token == webview.token:
            self.logger.debug("Token matches all is well")
            try:
                return f(self, *args, **kwargs)
            except Exception as e:
            ...
            self.logger.error("The request token doesn't match the server token")
            payload = StatusDTO(
